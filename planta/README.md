...
- **Setorizar** — cada mesa recebe uma cor de setor (Comercial, Operações, RH…).
...
- **Exportar / Importar JSON** — para levar a configuração para outro computador.

Tudo é salvo automaticamente no `localStorage` do navegador.
...
└── js/
    ...
    └── app.js            abas de ambiente, barra de ferramentas, init
```
